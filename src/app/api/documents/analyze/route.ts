...
async function extractPdfSections(
  buffer: ArrayBuffer
): Promise<{ sections: DocumentSection[]; pageCount: number }> {
  try {
...
export async function GET(request: Request) {
  try {
    ...
    console.log(
      `[API/analyze] GET: Fetching analysis for document ${documentId}`
    );
    ...
    const { data: document, error: documentError } = await getDocumentById(
      supabase,
      documentId,
      userId
    );
    ...
    const { data: fileData, error: fileError } = await downloadDocument(
      supabase,
      document.path
    );
    ...
    console.log(
      "[API/analyze] GET: Returning new analysis with",
      sections.length,
      "sections"
    );
...
export async function POST(request: Request) {
  try {
    ...
    const { documentId, question, highlightedSectionId, requestType } =
      requestData;
    ...
    console.log(
      `[API/analyze] POST: Analyzing document ${documentId} with request type: ${
